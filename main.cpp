...
vector<string> get_i2c_buses() {
    vector<string> i2c_buses;
    DIR* dir = opendir("/sys/bus/i2c/devices");
    if (dir == nullptr) {
        std::cerr << "Failed to open /sys/bus/i2c/devices directory" << std::endl;
        return i2c_buses;
    }

    dirent* entry;
    while ((entry = readdir(dir)) != nullptr) {
        // Check if the entry is a directory, starts with "i2c-" and name's length no longer than 8 symbols
        if (entry->d_type == DT_LNK && strncmp(entry->d_name, "i2c-", 4) == 0 && strlen(entry->d_name)<8) {
...
void print_buf_symbols(unsigned char* buffer, int start){
    cout<<"    ";
...
void i2c_device_dump(unsigned char* buffer, int device_addr, string bus){
    
    //table init
    cout<<"\nI2C Bus:\t"<<bus<<"  |   I2C-ADDR:\t"<<"0x"<< hex << setw(2) << setfill('0')<< device_addr << endl;
...
int main(){
    ...
    int rows_count=0x00;
    //Note: program dosn't check the bus with "i2c_open" function, due to fact that some buses don't open,
    //but devices which are connected to them could be available
        ...
        bus =i2c_open((*it).c_str());
		    ...
		    for(int i =0x00; i<0x80;i++){
                ...
                //if the connection was established successfully - make a dump
                        ...
                        //gathering devices
                        discovered_devices<<"I2C Bus: "<<*it<<"\t I2C-ADDR: 0x"<<std::hex<<std::setw(2)
